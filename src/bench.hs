...
------------------------------
evenFib maxVal = fLoop maxVal (0, 1, 0)
  where
    fLoop maxVal (prev, curr, evensum) =
      (if curr <= maxVal
         then fLoop maxVal (curr, newVal, sumIfEven)
         else (curr, newVal, sumIfEven))
      where
        newVal = prev + curr
        sumIfEven =
          (if even prev
...
divTest number primeList = divLoop number (primeList, number, [])
  where
    divLoop number (a, b, c) =
      (if (b > 1)
         then divLoop number (a, b `div` divisor, c ++ [divisor])
         else (a, b, c))
      where
        findDiv [x] = x
        findDiv (x:xs)
...
pb003 n =
  maximum (last3 (divTest n (takeWhile (<= n) $ (wheelSieve 6 :: [Integer]))))
  ...
  maximum
    ...
    | foo <-
        (transpose
           [ digit_list
           , digit_list
           , (tail ((tail (digit_list))))
           , tail ((tail ((tail (digit_list)))))
           ])
    ]
...
main =
  defaultMain
    ...
    , bgroup "pb003" [bench "ACC/600851475143" $ whnf pb002 600851475143]
    , bgroup
        "pb004"
...
--      bench "skip"  $ whnf pb004' 3
    ...
    , bgroup "pb005" [bench "10" $ whnf pb005 10, bench "20" $ whnf pb005 20]
    ]
